...
- 2026-08-04T05:15Z · `claude/prod-ui-auth-infra` · through `8b3f4ff` (+ layout fix) · Sprints 2, 4 and 5 complete — the lane's stated goal is done. Sprint 2: every merchant page resolves its shop (id_token → signed launch params → signed pf_shop cookie) and renders per-shop adapter data in real mode; demo rendering verified byte-identical on all nine routes via stash-diff; real mode verified end-to-end against local Supabase with a synced store; server actions persist COGS and notification emails; rollout creation honestly reports "not switched on" until the backend's POST /api/rollouts is integrated. Sprint 4: OAuth callback now reconciles webhooks and kicks off the initial sync via after() with a 10-minute freshness guard; /connect shows real staged progress with working retry. Sprint 5: preview deployment verified — /api/health 200 {ok, mode real, supabase reachable, all config true}; all merchant routes 200 through the access gate; CSP `frame-ancestors https://admin.shopify.com https://*.myshopify.com` served; App Bridge shell + api-key meta in served HTML; /api/auth 307s to the Shopify authorize URL with correct scopes; browser smoke 3/3 console-clean against a compiled demo server. Checks: typecheck pass, build pass, smoke 148/2 (unchanged baseline). Noted: Codex PR #2 and my branch overlap on 10 files (each lane's routes cross the other's list at the seams) — expected to resolve at the rebase-after-backend-merges step per the merge policy. Blockers: none on this lane.
