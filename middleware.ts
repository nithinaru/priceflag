 ...
 *     the OAuth callback), bounded to 24 hours against replay.
 *
...
const EXEMPT_EXACT = new Set(['/api/health', '/api/cron/evaluate', '/api/auth', '/api/auth/callback']);
...
/** Replay bound for hmac-signed launch params. */
...
async function isValidSessionToken(token: string, clientId: string, clientSecret: string): Promise<boolean> {
  ...
  return claims.iss.startsWith(claims.dest);
}
