...
# --- C10 / D-12: a probability that cannot say what it measures ---------------


def _monitor_with_assessments():
    """A fitted monitor plus one day of assessment, on golden data."""
    from priceflag_ml.data import load_golden
    from priceflag_ml.golden import GoldenConfig

    df = load_golden(GoldenConfig(n_skus=2, days=120, seed=11))
    cut = df["date"].max() - pd.Timedelta(days=5)
    pre, post = df[df["date"] <= cut], df[df["date"] > cut]
    monitor = CounterfactualMonitor(guardrail_drop_pct=0.30).fit(pre)
    return monitor, monitor.assess(post)


def test_units_guardrail_gets_the_probability():
    monitor, assessments = _monitor_with_assessments()
    rows = monitor.contract_rows(
        assessments, shop_domain="s1.myshopify.com",
        variant_gid="gid://shopify/ProductVariant/1",
        rollout_id="8f2b1c66-1f7e-4a4e-9c1f-2b3d4e5f6a7b",
        generated_at="2026-07-30T00:00:00Z",
        guardrail_metrics=("units",),
    )
    assert rows and all(r["breach_probability"] is not None for r in rows)


def test_a_revenue_guardrail_gets_null_not_the_units_probability():
    """D-12. The field carries no metric, and Lane B's guardrail engine reads
    it before it reads rule.metric — so a units probability would satisfy a
    revenue rule whose revenue is exactly on expectation. Null costs the
    noise-reduction C5 exists for; firing auto-rollback on a quantity nobody
    measured costs more."""
    monitor, assessments = _monitor_with_assessments()
    rows = monitor.contract_rows(
        assessments, shop_domain="s1.myshopify.com",
    ...
    assert rows and all(r["breach_probability"] is None for r in rows)


def test_mixed_metric_guardrails_also_get_null():
    monitor, assessments = _monitor_with_assessments()
    rows = monitor.contract_rows(
        assessments, shop_domain="s1.myshopify.com",
        variant_gid="gid://shopify/ProductVariant/1",
        rollout_id="8f2b1c66-1f7e-4a4e-9c1f-2b3d4e5f6a7b",
        generated_at="2026-07-30T00:00:00Z",
        guardrail_metrics=("units", "revenue"),
    )
    assert rows and all(r["breach_probability"] is None for r in rows)


def test_null_breach_probability_still_validates_against_the_contract():
    """The schema allows null; a suppressed probability must not make the row
    unwritable — the band itself is still the honest expectation."""
    import json
    import pathlib
    ...
    schema_path = pathlib.Path(__file__).resolve().parents[2] / "contracts" / "expected_band.schema.json"
    schema = json.loads(schema_path.read_text())
    for row in rows:
        jsonschema.validate(row, schema)


def test_default_is_units_so_existing_callers_are_unchanged():
    monitor, assessments = _monitor_with_assessments()
    default = monitor.contract_rows(
        assessments, shop_domain="s1.myshopify.com",
        variant_gid="gid://shopify/ProductVariant/1",
        rollout_id="8f2b1c66-1f7e-4a4e-9c1f-2b3d4e5f6a7b",
        generated_at="2026-07-30T00:00:00Z",
    )
    assert all(r["breach_probability"] is not None for r in default)
