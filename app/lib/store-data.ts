...
/** Read every catalog page; never accept PostgREST's default row ceiling. */
async function allProducts(adapter: StoreAdapter, shopId: string): Promise<Product[]> {
  const items: Product[] = [];
  for (let offset = 0; ; offset += PAGE_SIZE) {
    const page = await adapter.listProducts(shopId, { limit: PAGE_SIZE, offset });
    items.push(...page.items);
    if (items.length >= page.total || page.items.length < PAGE_SIZE) return items;
  }
}

/** A large or rolled-back rollout can create more than 1,000 journal rows. */
async function allJournalEntries(
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    const page = await adapter.listJournalEntries(shopId, {
...
function liveFromBundles(
  ...
  // "Completed" ends monitoring; it does not restore the old Shopify price.
  // The operational view must therefore follow unreverted writes, not only the
  // rollout state, or the emergency undo can appear disabled while our price is
  // still on the storefront.
  const live = bundles.filter(
    (bundle) => bundle.rollout.status !== "cancelled" && bundle.live.variants_live > 0,
  );
  const liveVariantGids = new Set(liveVariantGidsFrom(live));
...
export async function getRealLive(shop: Shop): Promise<LiveResponse> {
  ...
  return liveFromBundles(shop, bundles, products);
}
...
export async function getRealCatalog(shop: Shop): Promise<CatalogData> {
  ...
  const [products, orderDays, bundles] = await Promise.all([
    allProducts(adapter, shop.id),
...
export async function getRealJournal(shop: Shop): Promise<JournalEntry[]> {
  const adapter = getAdapter();
  return allJournalEntries(adapter, shop.id);
}
...
export async function getRealJournalForRollout(
  ...
  const adapter = getAdapter();
  return allJournalEntries(adapter, shop.id, rolloutId);
}
