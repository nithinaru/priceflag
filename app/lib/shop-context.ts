 ...
 *      (same scheme as the OAuth callback), bounded to 24h against replay.
 *   3. The `pf_shop` cookie — minted by `POST /api/auth/session` after a prior
 *      token verification. This is what every in-app navigation runs on.
...
/** Replay bound for hmac-signed launch params. Session tokens carry their own `exp`. */
const LAUNCH_MAX_AGE_SECONDS = 24 * 60 * 60;
...
export async function resolveShopForPage(searchParams: PageSearchParams): Promise<PageShopContext> {
  ...
  if (hasShopifyConfig()) {
    ...
    if (fromToken !== null) return { mode: 'real', shop: await adapter.getShopByDomain(fromToken) };

    const fromLaunch = shopFromLaunchParams(searchParams);
    if (fromLaunch !== null) return { mode: 'real', shop: await adapter.getShopByDomain(fromLaunch) };

    const cookieValue = (await cookies()).get(SHOP_COOKIE)?.value;
    if (cookieValue !== undefined) {
      const fromCookie = verifyShopCookie(cookieValue);
      if (fromCookie !== null) return { mode: 'real', shop: await adapter.getShopByDomain(fromCookie) };
    }
  }
  ...
  if (!isProductionRuntime()) {
    ...
    if (raw !== undefined) {
      try {
        return { mode: 'real', shop: await adapter.getShopByDomain(normalizeShopDomain(raw)) };
      } catch {
